...
class SqlUtil {
     ...
     * @returns {Promise<any>}
     */
    async fetchRow(fields, table, where, options, debug = false) {
     ...
     * @returns {Promise<any>}
     */
    async fetchOne(field, table, where, options, debug = false) {
        options = Object.assign({}, options || {}, { limit: 1 });
     ...
     * @returns {Promise<any[]>}
     */
    async fetchAll(fields, table, where, options, debug = false) {
     ...
     * @private
     */
    async _fetchAll(fields, table, where, options, debug = false) {
     ...
     */
    async fetchCount(table, where, debug = false) {
        where = this.buildWhere(where);
        if (where !== '') {
            where = ` WHERE ${where}`;
        }
     ...
     * @returns {Promise<any>}
     */
    async insert(table, data, debug = false) {
     ...
     * @returns {Promise<any>}
     */
    async update(table, data, where, debug = false) {
     ...
     */
    async delete(table, where, options, debug = false) {
        let addons = '';
        if (where === true) {
            // use with caution!
            where = '1=1';
